...
function derivatives = FuncAdsorption(t_ads,ADS,dz,N,L,ColumnParam,CondParam,isotherm_parameters)
...
% Define values to output matrix
    ...
    q2  = ADS(4*N+9:5*N+10);
    T   = ADS(5*N+11:6*N+12);
    TW  = ADS(6*N+13:7*N+14);

% Boundary Conditions
    %%% Inlet
    u(1)    = v0 ; 
    YA(1)     = (YA(2) + yA0*((v0*dz)/(2*DL)))/(1 + ((v0*dz)/(2*DL)));
    YB(1)     = (YB(2) + ((v0*dz)/(2*DL))*yB0)/(1 + ((v0*dz)/(2*DL)));
    
    MW_in = MW_air + (MW_water-MW_air)*YB(1);

    T(1)    = Ta;
    TW(1)     = Ta;
    P(1)    = ((viscous_term*u(1)*dz2) + P(2))/(1-(dz/2/R/Ta)*(1.75*(1-eb)*MW_in/dp/eb)*u(1)^2);   
...
% 4) 2nd derivatives
 ...
 % 5) Velocities at the boundary interface
    ...
    kinetic_term(2:N+1) = (1.75.*(1-eb).*rho_gh(2:N+1).*(MW_air+(MW_water-MW_air).*yBh(2:N+1)))./(eb*dp) ;    
    discriminant = abs(viscous_term^2 + (4*kinetic_term(2:N+1).*abs(dpdzh(2:N+1)))) ;
    u(2:N+1)       = -sign(dpdzh(2:N+1)).*(-viscous_term + sqrt(discriminant))./(2.*kinetic_term(2:N+1)) ; 
...
%    1.1 Calculate equilibrium molar loading        
        % qstar = Isotherm(YA, YB, P, T, CondParam, isotherm_parameters);
