...
function [MECH_ENERGY, CO2_PRODUCTIVITY, gH2Olost_gCO2cap, gH2Oused_gCO2cap, CO2_Purity, CSS_ok, i_end] = VMS_MainCycle(N, RH, t_cs, t_vs, t_des, vair, vvs)
tic 
...
% Input parameters
    InputParameters     = ProcessInputParameters(L,N,RH,t_cs,t_vs,t_des,vair, vvs);
...
% % Column initialization (Initial Conditions (IC))
%     qstar               = IsothermMS(0.02, 0.98, PL, 278, CondParam, isotherm_parameters)  ; %(yCO2,yH20,Press, Temp, isotherm input)
%     x0                  = zeros(7*N+14,1) ;                 % Initialize the size of the IC matrix
%     x0(1:N+2)           = 0.02            ;                 % Initial conditions for CO2 mole fraction
%     x0(N+3:2*N+4)       = 0.98            ;                 % Initial conditions for H20 mole fraction
...
%     x0(5*N+11:6*N+12)   = 278             ;                 % Initial conditions for total temperature
%     x0(6*N+13:7*N+14)   = 278             ;                 % Initial conditions for wall temperature
     
% Column initialization (Initial Conditions (IC))
    qstar               = IsothermMS(yA0, yB0, PH, 298, CondParam, isotherm_parameters)  ; %(yCO2,yH20,Press, Temp, isotherm input)
    x0                  = zeros(7*N+14,1) ;                 % Initialize the size of the IC matrix
    ...
    x0(3*N+7:4*N+8)     = qstar(1)        ;                 % Initial conditions for CO2 reaction rate
    x0(4*N+9:5*N+10)    = qstar(2)        ;                 % Initial conditions for H20 reaction rate
    x0(5*N+11:6*N+12)   = 298             ;                 % Initial conditions for total temperature
...
max_cycles = 200;
cycle_data   = zeros(max_cycles,8) ; % This will hold the cycle number, CSS, and mass balance
CSS_tolerance = 0.005;
MBS_tolerance = 0.005;
...
for i = 1:max_cycles
  ...
  %%% 1. SIMULATE PRESSURIZATION STEP [OPEN--CLOSED]
        ...
        MW_in           = MW_air + (MW_water-MW_air).*a(:, N+3)                             ; % molecular weight of species entering
        rho_g           = a(:,2*N+5)/(R*Tatm)                                                 ; % density in the gas phase
        alpha           = (1.75*(1-eb).*rho_g.*MW_in)./(eb*dp)                             ; % kinetic term for ergun equation
        disc            = beta^2 - (4*alpha.*(a(:,2*N+6)-a(:,2*N+5))*(2/dz)) ; % discriminant of the quadratic equation
        u               = (-beta + sqrt(disc))./(2*alpha)                    ; % inlet velocity  
        a(:, 1)         = (a(:,2) + yA0.*((u.*dz)./(2*DL)))./ (1 + ((u.*dz)./(2*DL)))       ; % CO2 mole fraction
        a(:, N+3)       = (a(:,N+4) + yB0.*((u.*dz)./(2*DL)))./ (1 + ((u.*dz)./(2*DL)))     ; % H2O mole fraction         
  ...
  %%% 2. SIMULATE CO2 SORPTION STEP [OPEN--OPEN]
        ...
        rho_g           = PH/(R*Tatm)                                                     ; 
        convDiff      = rho_g*((eb*vair*C_pg*dz)/(2*Kz))                              ;
        ...
        b(:, 2*N+5)     = ((beta*vair*dz2) + b(:, 2*N+6))...
                          /(1-(dz/2/R/b(:, 5*N+11))*(1.75*(1-eb)*MW_in/dp/eb)*vair^2)   ;  
  ...
  %%% 3. SIMULATE AIR EVACUATION STEP [CLOSED--OPEN]
        ...
        x30             = c(end, :)'     ; % Final state of evacuation becomes initial state for the next step      
 ...
 %%% 4. SIMULATE CO2 WATER EVAPORATION STAGE [OPEN--OPEN]
        ...
        % Update the boundary conditions (clean up results from simulation)
        %%% Column Inlet
        d(:, 2*N+5)     = PressureVaporSat(Tsat)            ; % Pressure
        idx             = find(d(:, 2*N+6) > d(:,2*N+5))    ; % Find values of P(2) > P(1)
        d(idx, 2*N+5)   = d(idx, 2*N+6)                     ;        
        d(:, 1)         = yA0                               ; % yCO2(1)
        d(:, N+3)       = yB0                               ; % yH2O(1) 
        d(:, 3*N+7)     = d(:, 3*N+8)                       ; % qCO2(1)  = qCO2(2)
        d(:, 4*N+9)     = d(:, 4*N+10)                      ; % qH20(1)  = qH20(2)                
        MW_in           = MW_air + (MW_water-MW_air).*d(:, N+3);
        rho_g           = d(:,2*N+5)/(R*Tsat)               ;  
        alpha    = (1.75*(1-eb).*rho_g.*MW_in)./(eb*dp) ;
        dpdz            = (d(:,2*N+6)-d(:,2*N+5))/(dz2)     ; % (P(2)-P(1))/dz/2
        disc            = beta^2 - (4*alpha.*dpdz) ;
        u               = (-beta + sqrt(disc))./(2*alpha) ;   % inlet velocity  
        convDiff      = rho_g.*((eb.*u.*C_pg*dz)./(2*Kz)) ;
        ...
        d(:, 6*N+12)    = d(:, 6*N+11)                      ;
        MW_out          = MW_air + (MW_water-MW_air).*d(:, 2*N+4)           ; % T(N+2) = T(N+1)        
        d(: ,3*N+6)     = (-(beta*vvs*dz2) + d(: ,3*N+5))...
                          ./(1 + (dz/2/R./d(:, 5*N+11)).*(1.75*(1-eb).*MW_out/dp/eb).*vvs^2);                                                   
  ...
  %%% 5. SIMULATE CO2 REMOVAL STEP [CLOSED--OPEN]
        ...
        % Re-define mole fraction of water to original feed value for next cycle. 
        RH        = RH2                     ; % air RH
   ...
   % Cyclic Steady State Verification
        ...
        mb_error = MassBalance.CO2_cycle_balance ;
    ...
    CSS_ok = all(abs(dyCO2) < CSS_tolerance) && all(abs(dyH2O) < CSS_tolerance) && ...
    ...
    % Check if the CSS tolerance criteria were met for 5 consecutive cycles
    if consecutive_cycles_met >= 5
        fprintf('CSS achieved: breaking loop at cycle %d of %d.\n', i, max_cycles);
...
CSS_ok = break_condition_met;   % true only if CSS achieved
i_end = i;
% PERFORMANCE METRICS
  Objectives = MechanicalEnergy(a, b, c, d, e, t1, t2, t3, t4, t5, RH, N, L, ColumnParam, CondParam, Times);
