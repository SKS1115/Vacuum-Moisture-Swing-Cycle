...
function derivatives = FuncVStripping(t_vs,VS,x30,yA0,yB0,dz,N,L,ColumnParam,CondParam,isotherm_parameters)
...
% Extract Relevant Isotherm Parameters
    dUb_CO2     = -20000    ; % Heats of sorption of CO2 during stripping
    dUb_H2O     = -42000    ; % Heats of sorption of H2O during stripping
...
% Define values to output matrix
    YA  = VS(1:N+2)        ; % CO2 mole fraction
    YB  = VS(N+3:2*N+4)    ; % H2O mole fraction
    P   = VS(2*N+5:3*N+6)  ; % Pressure
    q1  = VS(3*N+7:4*N+8)  ; % CO2 loading
    q2  = VS(4*N+9:5*N+10) ; % H2O loading
    T   = VS(5*N+11:6*N+12); % Temperature
    TW  = VS(6*N+13:7*N+14); % Wall Temperature

% Boundary Conditions
    %%% Column Inlet
    YA(1)     = yA0;
    YB(1)     = yB0;
    P(1)      = PressureVaporSat(Tsat);
    TW(1)     = Tatm;
    if P(2) > P(1)
       P(1) = P(2);
    end
    ...
    dsic1     = beta^2 - (4*alpha(1).*(P(2)-P(1))*(2/dz)) ;
    u(1)      = (-beta + sqrt(dsic1))./(2*alpha(1)) ;     
    
    convDiff = (eb*u(1)*rho_g(1)*C_pg*(dz/2))/Kz;
    T(1)      = (T(2) + (Tsat*convDiff))/(1+convDiff);
    ...
    P(end)    = (-(beta*u(end)*dz2) + P(end-1))/(1+(dz/2/R/T(end))*(1.75*(1-eb)*MW_out/dp/eb)*u(end)^2);   
...
% 5) Velocities at the boundary interface
    ...
    alpha(2:N) = (1.75*(1-eb).*rho_g(2:N).*(MW_air+(MW_water-MW_air).*yBh(2:N)))./(eb*dp) ;   
...
%    1.2 Calculate the uptake rate  
        ...
        dqdt2(2:N+1) = kH2O_ads.*(qstar2(2:N+1)-q2(2:N+1));
