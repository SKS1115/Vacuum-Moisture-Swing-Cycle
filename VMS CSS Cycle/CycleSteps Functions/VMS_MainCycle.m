...
% Column initialization (Initial Conditions (IC))
    ...
    x0(5*N+11:6*N+12)   = Ta              ;                 % Initial conditions for total temperature
    x0(6*N+13:7*N+14)   = Ta              ;                 % Initial conditions for wall temperature
...
%   state variable is less than the prescribed CSS criteria. 
 ...
 for i = 1:cycles
  ...
  %%% 1. SIMULATE PRESSURIZATION STEP [OPEN--CLOSED]
        ...
        rho_g           = a(:,2*N+5)/(R*Ta)                                                 ; % density in the gas phase
        ...
        a(:, 5*N+11)    = (a(:, 5*N+12) + (Ta.*convDiff(:)))./(1+convDiff(:))           ; % T(1) (Dankwerts BC)
  ...
  %%% 2. SIMULATE CO2 SORPTION STEP [OPEN--OPEN]
        ...
        b(:, 5*N+11)    = (b(:, 5*N+12) + (Ta.*convDiff(:)))./(1+convDiff(:))       ; % T(1) (Dankwerts BC     
 ...
 %%% 4. SIMULATE CO2 WATER EVAPORATION STAGE [OPEN--OPEN]
        ...
        d(: ,3*N+6)     = (-(beta*u_vp*dz2) + d(: ,3*N+5))...
                          ./(1 + (dz/2/R./d(:, 5*N+11)).*(1.75*(1-eb).*MW_out/dp/eb).*u_vp^2);                                                   
  ...
  %%% 5. SIMULATE CO2 REMOVAL STEP [CLOSED--OPEN]
        ...
        RH        = RH2                     ; % air RH
        PH2O_atm  = PressureVaporSat(Ta)    ; % Partial pressure of air H2O
...
disp('CO2 Mass Balance Table'); disp(MassBalance.CO2_MassBalanceTable);
disp('H2O Mass Balance Table'); disp(MassBalance.H2O_MassBalanceTable);
% disp('Energy Balance Table'); disp(EnergyBalance);
% disp('Performance Metrics'); disp(PerformanceMetrics);
