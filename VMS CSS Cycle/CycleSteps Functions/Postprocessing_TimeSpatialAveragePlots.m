...
% Extract and define all the calculated unknown parameters for each step
    ...
    Vel_A     = (-dz)*(Viscous)*(a(:, 2*N+6:3*N+6)-a(:, 2*N+5:3*N+5))  ;
    ...
    Vel_B     = (-dz)*(Viscous)*(b(:, 2*N+6:3*N+6)-b(:, 2*N+5:3*N+5))  ;
    ...
    Vel_C     = (-dz)*(Viscous)*(c(:, 2*N+6:3*N+6)-c(:, 2*N+5:3*N+5))  ;
    ...
    Vel_E     = (-dz)*(Viscous)*(e(:, 2*N+6:3*N+6)-e(:, 2*N+5:3*N+5))  ;
  
    yCO2_F    = f(:,1:N+2)        ; yH2O_F    = f(:,N+3:2*N+4)        ; 
    Press_F   = f(:,2*N+5:3*N+6)  ; Temp_F    = f(:, 5*N+11:6*N+12)   ;
    qCO2_F    = f(:,3*N+7:4*N+8)  ; qH2O_F    = f(:,4*N+9:5*N+10)     ;
    yN2_F     = 1-(yCO2_F+yH2O_F);
    PCO2_F    = Press_F.*yCO2_F   ; PH2O_F    = Press_F.*yH2O_F  ;
    Vel_F     = (-dz)*(Viscous)*(f(:, 2*N+6:3*N+6)-f(:, 2*N+5:3*N+5))  ;
...
steps = {
    'Pressurization', t1, qCO2_A, qH2O_A, yCO2_A, yH2O_A, Temp_A, Press_A, PCO2_A, PH2O_A, Vel_A;
    'CO_2 Sorption', t2, qCO2_B, qH2O_B, yCO2_B, yH2O_B, Temp_B, Press_B, PCO2_B, PH2O_B, Vel_B;
    'Air Evacuation', t3, qCO2_C, qH2O_C, yCO2_C, yH2O_C, Temp_C, Press_C, PCO2_C, PH2O_C,Vel_C;    
    'Vapor Stripping', t5, qCO2_E, qH2O_E, yCO2_E, yH2O_E, Temp_E, Press_E, PCO2_E, PH2O_E,Vel_E;
    'Final Desorption', t6, qCO2_F, qH2O_F, yCO2_F, yH2O_F, Temp_F, Press_F, PCO2_F, PH2O_F, Vel_F;
};
...
for step_idx = 1:size(steps, 1)
    ...
    Vel = arrayfun(@(i) trapz(z2, Vel_data(i, :)) / L, 1:length(t));
    ...
    cumulative_Vel = [cumulative_Vel, Vel];
end
...
y_labels = {'qCO_2 [mmol/cm^3]', 'qH_2O [mmol/cm^3]', 'yCO_2 [-]', 'yH_2O [-]', ...
    'Temperature [K]', 'Pressure [kPa]', 'PCO_2 [kPa]', 'PH_2O [kPa]', 'Velocity [m/s]'};
parameters = {cumulative_qCO2*1e-3, cumulative_qH2O*1e-3, cumulative_yCO2, ...
    cumulative_yH2O, cumulative_Temp, cumulative_Press*1e-3, cumulative_PCO2*1e-3, cumulative_PH2O*1e-3, cumulative_Vel};
