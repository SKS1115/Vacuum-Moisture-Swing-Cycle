...
%% function to solve Temperature, Pressure and Mole fraction rate during the Pressurization Stage
...
% Define values to output matrix
    YA  = PRZ(1:N+2)        ; % CO2 mole fraction
    YB  = PRZ(N+3:2*N+4)    ; % H2O mole fraction
    P   = PRZ(2*N+5:3*N+6)  ; % Pressure
    q1  = PRZ(3*N+7:4*N+8)  ; % CO2 loading
    q2  = PRZ(4*N+9:5*N+10) ; % H2O loading
    T   = PRZ(5*N+11:6*N+12); % Temperature
    TW  = PRZ(6*N+13:7*N+14); % Wall Temperature
...
%% Boundary Conditions
    ...
    rho_g    = P(1)/(R*Tatm);  
    alpha(1) = (1.75*(1-eb).*rho_g*MW_in)./(eb*dp) ;
    ...
    convDiff = (eb*u(1)*rho_g*C_pg*(dz/2))/Kz;
    T(1)     = (T(2) + (Tatm*convDiff))/(1+convDiff);  
    TW(1)    = Tatm;
...
% 4) 2nd derivatives
 ...
 % 5) Velocities at the boundary interface
    ...
    alpha(1:N+1) = (1.75.*(1-eb).*rho_gh(1:N+1) ...
                        .*(MW_air+(MW_water-MW_air).*yBh(1:N+1)))./(eb*dp) ;    
    disc      = abs(beta^2 + (4*alpha(1:N+1).*abs(dpdzh(1:N+1)))) ;
    u(1:N+1)  = -sign(dpdzh(1:N+1)).*(-beta + sqrt(disc))...
                ./(2.*alpha(1:N+1)) ; 
...
% 5) Calculate H2O Mole fraction 
        YB_PVT  = (yBh(1:N+1).*Ph(1:N+1).*u(1:N+1))./Th(1:N+1);           
