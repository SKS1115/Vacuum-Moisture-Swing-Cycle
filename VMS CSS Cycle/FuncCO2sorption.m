...
%% function to solve Temperature, Pressure and Mole fraction rate during the CO2 sorption Stage
% NB: This function does not include half cells.

function derivatives = FuncCO2sorption(t_cs,ADS,dz,N,L,ColumnParam,CondParam,isotherm_parameters)
...
%% Initialize all main parameters
    ...
    derivatives   = zeros(length(ADS),1);
    dyAdt    = zeros(N+2,1);
    dyAdt1   = zeros(N+2,1);
    dyAdt2   = zeros(N+2,1);
    dyAdt3   = zeros(N+2,1);
...
% Define values to output matrix
    YA  = ADS(1:N+2)        ; % CO2 mole fraction
    YB  = ADS(N+3:2*N+4)    ; % H2O mole fraction
    P   = ADS(2*N+5:3*N+6)  ; % Pressure
    q1  = ADS(3*N+7:4*N+8)  ; % CO2 loading
    q2  = ADS(4*N+9:5*N+10) ; % H2O loading
    T   = ADS(5*N+11:6*N+12); % Temperature
    TW  = ADS(6*N+13:7*N+14); % Wall Temperature

%% Boundary Conditions
    %%% Column Inlet
    u(1)    = vair ; 
    YA(1)   = (YA(2) + yA0*((vair*dz)/(2*DL)))/(1 + ((vair*dz)/(2*DL)));
    YB(1)   = (YB(2) + ((vair*dz)/(2*DL))*yB0)/(1 + ((vair*dz)/(2*DL)));
    T(1)    = Tatm;
    TW(1)   = Tatm;
    MW_in   = MW_air + (MW_water-MW_air)*YB(1);    
    P(1)    = ((beta*u(1)*dz2) + P(2))/(1-(dz/2/R/Tatm)*(1.75*(1-eb)*MW_in/dp/eb)*u(1)^2); 
    if P(2) > P(1)
        P(1) = P(2) ;
    end
...
% 4) 2nd derivatives
 ...
 % 5) Velocities at the boundary interface
    rho_gh(2:N+1)= Ph(2:N+1)./(R.*Th(2:N+1));
    alpha(2:N+1) = (1.75.*(1-eb).*rho_gh(2:N+1) ...
                   .*(MW_air+(MW_water-MW_air).*yBh(2:N+1)))./(eb*dp) ;    
    disc         = abs(beta^2 + (4*alpha(2:N+1).*abs(dpdzh(2:N+1)))) ;
    u(2:N+1)     = -sign(dpdzh(2:N+1)).*(-beta + sqrt(disc))./(2.*alpha(2:N+1)) ; 
...
%    1.2 Calculate the uptake rate  
        dqdt1(2:N+1) = kCO2_ads.*(qstar1(2:N+1)-q1(2:N+1));
...
%         sink = (rho_s*C_ps) + (C_pa_CO2.*q1(2:N+1)) + (Cpa_H2O.*q2(2:N+1)); 
%         Pv   = Ph(1:N+1).*u(1:N+1);
...
%         dTdt3(2:N+1) = (1./sink).*T(2:N+1).*(C_pa_CO2.*dqdt1(2:N+1) + Cpa_H2O.*dqdt2(2:N+1))  ; % Temperature change due to reaction
...
dTdt5(2:N+1) = ((-dHCO2./sink).*dqdt1(2:N+1)) + ((-dHH20./sink).*dqdt2(2:N+1)); 
...
%%    
% 4)  Calculate CO2 Mole fraction                                      % Pressure divided by Temperature at all wall nodes
