...
function InputParameters = ProcessInputParameters(N,RH,ID,OD,L,vair,vvs,t_cs,t_vs,t_des)

% 1) Define column properties and relevant constants
% 2) Define case operating conditions including kinetics
% 3) Define isotherms properties
% 4) Define the time for each cycle
% 5) Distribute the values defined to new variables e.g. Param(1) =
% R (gas constant) 
% 6) Combine all lists into one variable of cells that can be used in other
% scripts. Call this variable InputParams.

%``````````````````````````Column Properties`````````````````````````````
    r_in    = ID/2      ;            % Inner column radius (units:m)
    r_out   = OD/2      ;            % Outer column radius (units:m)
    eb      = 0.35      ;            % bed voidage (units: dimensionless)commandhistory
    ep      = 0.238     ;            % particle voidage (units: dimensionless)
    rp      = 4e-4      ;            % IRA 900 particle radius (units: m)
    tau     = 3         ;            % adsorbent tortuosity (units: dimensionless);
...
%`````````````````````````` Properties and Constants````````````````````````
    ...
    C_pw     = 502       ;        % specific heat capacity of column wall {units: J/kg*K}. Co relates to stainless steel 304
    mu       = 1.72e-5   ;        % fluid viscosity {units: kg/ms}
    Dm       = 2.05e-5   ;        % effective macro pore diffusivity {units: m2/s}
    ...
    lambda   = 0.5       ;        % parameter used to determine rate of pressure change {units: 1/s}
    dp       = rp*2      ;        % diameter of the adsorbent particle {units: m}
    tho      = 10e-10    ;        % constant used in van leer expression 
    MW_air   = 0.02896   ;
    MW_water = 0.018015  ; 

%``````````````````````````Operating Conditions for case study```````````````
    Tsat     = 298.15  ;          % feed temperature of water during vapor stripping {units: K}
...
%```````````````````````Isotherm Parameters for H20````````````````````` 
    qm_H2O  = 11.4360   ;         % H20 loading capacity mono-layer for IRA900 [mol/kg]
    E_H2O   = 38.6      ;         % Experimentally fitted constant used in 1st layer heat of adsorption [J/mol/K]
    F_H2O   = 59338     ;         % Experimentally fitted constant used in 1st layer heat of adsorption [J/mol] 
    G_H2O   = 50.4      ;         % Experimentally fitted constant used in 1st layer heat of adsorption [J/mol/K]
    H_H2O   = 58322     ;         % Experimentally fitted constant used in 1st layer heat of adsorption [J/mol]
