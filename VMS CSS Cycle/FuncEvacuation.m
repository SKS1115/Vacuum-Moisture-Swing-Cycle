...
function derivatives = FuncEvacuation(t_ev,EVS,dz,N,L,ColumnParam,CondParam,isotherm_parameters)
...
% Define values to output matrix
    YA  = EVS(1:N+2)        ; % CO2 mole fraction
    YB  = EVS(N+3:2*N+4)    ; % H2O mole fraction
    P   = EVS(2*N+5:3*N+6)  ; % Pressure
    q1  = EVS(3*N+7:4*N+8)  ; % CO2 loading
    q2  = EVS(4*N+9:5*N+10) ; % H2O loading
    T   = EVS(5*N+11:6*N+12); % Temperature
    TW  = EVS(6*N+13:7*N+14); % Wall Temperature
...
% 4) 2nd derivatives
 ...
 % 5) Velocities at the boundary interface
   ...
   u(1:N+1)     = -sign(dpdzh(1:N+1)).*(-beta + sqrt(disc))./(2.*alpha(1:N+1)) ; 
