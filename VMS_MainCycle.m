...
% 1) create .mat file for IsothermData and arrange in specific order so you
% can call this file in ProcessInputParameters when defining Isotherm
% Parameters (qsb, qsd etc)
% 2) create .mat file for material property to include properties of the
% sorbent like delta U and rho_s. You will call this file in
% ProcessInputParameters
% 3) In this script, define N and run time
% 4) call isotherm function to solve for q
% 5) Call adsorption function and use ode23s solver to find solution

% y(1), q(1) etc are inlet boundary values
% y(N+2), q(N+2) etc are outlet boundary values
% y(2:N+1), q(N+2) etc are outlet boundary values

%``````````````````````````Domain Setup```````````````
prompt = "discretization size (+ intergers) =";
N = input(prompt);             % Number of cells...number of rows (units: dimensionless)
prompt1 = "Relative Humidity (%) =" ;
RH = input(prompt1);
RH2 = RH;
prompt2 = "# Cycles (%) =" ;
 ...
 %%% 4. SIMULATE CO2 WATER EVAPORATION STAGE [OPEN--OPEN]
  ...
  %%% 5. SIMULATE CO2 REMOVAL STEP [CLOSED--OPEN]
   ...
   % Check Cyclic Steady State
        ...
        MassBalance = MassBalanceCheck(a,b,c,e,f,t1,t2,t3,t5,t6,N,L,ColumnParam,CondParam) ;
        mass_balance = MassBalance.CO2_cycle_balance ;
        cycle_data(i,:) = round([i, dyCO2, dyH2O, dPress, dqCO2,dqH2O,dTemp, abs(mass_balance-1)], 2, 'significant');  % Store cycle number, CSS, and mass balance error
        
        EnergyBalance = EnergyBalanceCheck(a,b,c,e,f,t1,t2,t3,t5,t6,N,L,ColumnParam,CondParam,isotherm_parameters) ;
