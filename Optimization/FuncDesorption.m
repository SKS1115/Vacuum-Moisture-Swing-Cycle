...
% Boundary Conditions
    ...
    rho_g  = P(end)/(R*T(end));
    kinetic_term(end)  = (1.75*(1-eb).*rho_g*MW_out)./(eb*dp) ;
    dsmt2     = abs(viscous_term^2 + (4*kinetic_term(end).*abs(P(end)-P(end-1))*(2/dz))) ;
    u(end)    = -sign(P(end)-P(end-1))*(-viscous_term + sqrt(dsmt2))./(2*kinetic_term(end)) ; 
...
% 4) 2nd derivatives
 ...
 % 5) Velocities at the boundary interface
    ...
    kinetic_term(2:N) = (1.75*(1-eb).*rho_g(2:N))./(eb*dp) ;    
...
% 5) Calculate H2O Mole fraction 
   ...
   % Boundary Derivatives 
    ...
    dpdt(1)    = dpdt(2);
    dpdt(end)  = lambda*(PL-EndingP)*exp(-lambda*t_des);
    dqdt1(1)   = 0;
    dqdt1(end) = 0;
    dqdt2(1)   = 0;
    dqdt2(end) = 0;
