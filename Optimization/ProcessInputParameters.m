...
function InputParameters = ProcessInputParameters(N,RH,t_ad,t_vp,t_de,v_ads, v_vp)
...
%``````````````````````````Column Properties`````````````````````````````
    L       = 0.01      ;            % The length of the reactor column (units: m)
    ...
    tortuosity = 3      ;            % adsobent tortuosity (units: dimensionless);
