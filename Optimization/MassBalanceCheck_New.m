...
function MassBalance = MassBalanceCheck_New(a, b, c, e, f, t1, t2, t3, t5, t6, N, L, ColumnParam, CondParam)
    ...
    [yCO2_F, yH2O_F, Press_F, Temp_F, qCO2_F, qH2O_F, yN2_F] = extract_params(f);
    ...
    function [Uin, Uout, JMB_CO2, molCO2in, molCO2out, netCO2, netCO2_solid, netCO2_gas, molCO2_sorbed, nCO2in, nCO2out] = CO2_balance(Press, Temp, yCO2, yH2O, qCO2, ~, time)
        ...
        kinetic_term1  = (1.75*(1-eb).*rho_g1.*MW_in)./(eb*dp) ;
        dsmt1   = viscous_term^2 - (4*kinetic_term1.*(Press(:,2) - Press(:,1)).*(2/dz)) ;
        Uin     = max(((-viscous_term + sqrt(dsmt1)) ./ (2*kinetic_term1)), 0) ; 

        MW_out = 0.02896 + (-0.0109)*yH2O(:,end);
        rho_g2    = Press(:,end)./(R*Temp(:,end));  
        kinetic_term2  = (1.75*(1-eb).*rho_g2.*MW_out)./(eb*dp) ;
        dsmt2   = viscous_term^2 - (4*kinetic_term2.*(Press(:,end) - Press(:,end-1)).*(2/dz)) ;
        Uout    = max(((-viscous_term + sqrt(dsmt2)) ./ (2*kinetic_term2)), 0) ;         
    ...
    [Uin_vp , Uout_vp , JMB_vp_CO2 , molCO2in_vp , molCO2out_vp , netCO2_vp , netCO2_SP_vp , netCO2_GP_vp , molCO2_SB_vp , ~ , nCO2out_vp ]     = CO2_balance(Press_E, Temp_E, yCO2_E, yH2O_E, qCO2_E, qH2O_E, t5);
    [Uin_des, Uout_des, JMB_des_CO2, molCO2in_des, molCO2out_des, netCO2_des, netCO2_SP_des, netCO2_GP_des, molCO2_SB_des, ~, nCO2out_des ]     = CO2_balance(Press_F, Temp_F, yCO2_F, yH2O_F, qCO2_F, qH2O_F, t6);

    % % CO2 Outputs
    % CO2_molarflow     = {nCO2in_pz, nCO2in_ads, nCO2out_ev, nCO2out_vp, nCO2out_des};
    % CO2_moles         = {molCO2in_pz, molCO2in_ads, molCO2out_ads, molCO2out_ev, molCO2out_vp, molCO2out_des}; 
...
% Compute Mass Balance for Each Step
    ...
    [JMB_vp_H2O , molH2Oin_vp , molH2Oout_vp , netH2O_vp , netH2O_SP_vp , netH2O_GP_vp , molH2O_SB_vp , nH2Oin_vp, nH2Oout_vp]     = H2O_balance(Press_E, Temp_E, yCO2_E, yH2O_E, qCO2_E, qH2O_E, t5);
    [JMB_des_H2O, molH2Oin_des, molH2Oout_des, netH2O_des, netH2O_SP_des, netH2O_GP_des, molH2O_SB_des, ~, nH2Oout_des       ]     = H2O_balance(Press_F, Temp_F, yCO2_F, yH2O_F, qCO2_F, qH2O_F, t6);
...
% Compute Mass Balance for Each Step
    ...
    [~ , molN2out_vp , ~, nN2out_vp]     = N2_balance(Press_E, Temp_E, yCO2_E, yH2O_E, qCO2_E, qH2O_E, yN2_E, t5);
    [~, molN2out_des, ~, nN2out_des]     = N2_balance(Press_F, Temp_F, yCO2_F, yH2O_F, qCO2_F, qH2O_F, yN2_F, t6);
 ...
 % Final Output
    CO2_molarflow = {nCO2in_pz, nCO2in_ads, nCO2out_ev, nCO2out_vp, nCO2out_des};
    H2O_molarflow = {nH2Oin_pz, nH2Oin_ads, nH2Oout_ev, nH2Oout_vp, nH2Oout_des};    
    N2_molarflow  = {nN2in_pz, nN2in_ads, nN2out_ev, nN2out_vp, nN2out_des};
    CO2_moles     = {molCO2in_pz, molCO2in_ads, molCO2out_ads, molCO2out_vp,molCO2out_des};
    H2O_moles     = {molH2Oin_pz, molH2Oin_ads, molH2Oin_vp, molH2Oout_ads, molH2Oout_vp, molH2Oout_des};  
    N2_moles      = {molN2out_vp, molN2out_des}; 
    ...
    MassBalance.CO2_molarflow          = CO2_molarflow         ; 
    MassBalance.CO2_moles              = CO2_moles             ;
    MassBalance.H2O_molarflow          = H2O_molarflow         ; 
    MassBalance.H2O_moles              = H2O_moles             ; 
    MassBalance.N2_molarflow           = N2_molarflow          ; 
    MassBalance.N2_moles               = N2_moles              ; 
end
