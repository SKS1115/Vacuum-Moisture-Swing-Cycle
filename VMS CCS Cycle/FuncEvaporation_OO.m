...
%% function to solve Temperature, Pressure and Mole fraction rate during the Vacuum Evaporation Stage

function derivatives = FuncEvaporation_OO(t_evap2,EVAP,x30,yA0,yB0,dz,N,L,ColumnParam,CondParam,isotherm_parameters)
...
% Extract Input Parameters
    ...
    kH2O_ads = CondParam(32); 
    MW_air   = CondParam(34);
    MW_water = CondParam(35); 


% Extract Relevant Isotherm Parameters
    dUb_CO2     = -20000;%isotherm_parameters(4);
    dUb_H2O     = -42000;%isotherm_parameters(10);
...
% Initialize all main parameters
    ...
    derivatives   = zeros(length(EVAP),1);
    dyAdt    = zeros(N+2,1);
    dyAdt1   = zeros(N+2,1);
    dyAdt2   = zeros(N+2,1);
    dyAdt3   = zeros(N+2,1);
...
% Define values to output matrix
    ...
    P   = EVAP(2*N+5:3*N+6);
    q1  = EVAP(3*N+7:4*N+8);
    q2  = EVAP(4*N+9:5*N+10);
    T   = EVAP(5*N+11:6*N+12);
    TW  = EVAP(6*N+13:7*N+14);
...
% Boundary Conditions
    ...
    P(1)      = PressureVaporSat(Tf);
    TW(1)     = Ta;
    if P(2) > P(1)
       P(1) = P(2);
    end
    rho_g(1)     = P(1)/(R*Tf);  
    kinetic_term(1)  = (1.75*(1-eb).*rho_g(1)*MW_in)./(eb*dp) ;
    dsmt1   = viscous_term^2 - (4*kinetic_term(1).*(P(2)-P(1))*(2/dz)) ;
    u(1)    = (-viscous_term + sqrt(dsmt1))./(2*kinetic_term(1)) ; 
    
    Constant2 = (eb*u(1)*rho_g(1)*Cpg*(dz/2))/Kz;
    T(1)      = (T(2) + (Tf*Constant2))/(1+Constant2);
    ...
    P(end)    = (-(viscous_term*u(end)*dz2) + P(end-1))/(1+(dz/2/R/T(end))*(1.75*(1-eb)*MW_out/dp/eb)*u(end)^2);   
...
% 5) Velocities at the boundary interface
    ...
    kinetic_term(2:N) = (1.75*(1-eb).*rho_g(2:N).*(MW_air+(MW_water-MW_air).*yBh(2:N)))./(eb*dp) ;   
    discriminant = viscous_term^2 - (4*kinetic_term(2:N).*dpdzh(2:N)) ;
    u(2:N)       = (-viscous_term + sqrt(discriminant))./(2.*kinetic_term(2:N)) ; 
...
%     2.2 Calculate the Total Temperature   
        % sink        = (rho_s*C_ps) + C_pa_CO2.*(q1(2:N+1)+q2(2:N+1));     
        ...
        % dTdt3(2:N+1) = (C_pa_CO2./sink).*T(2:N+1).*(dqdt1(2:N+1)+dqdt2(2:N+1))  ; % Temperature change due to reaction
...
% 4)  Calculate CO2 Mole fraction
        ...
        dyAdt1(2:N+1)  = DL(2:N+1).*(d2yAdz2(2:N+1)+(dyAdz(2:N+1).*dpdz(2:N+1)./P(2:N+1))-(dyAdz(2:N+1).*dTdz(2:N+1)./T(2:N+1)))                        ;
...
% 5) Calculate H2O Mole fraction 
        ...
        dyBdt1(2:N+1)  = DL(2:N+1).*(d2yBdz2(2:N+1)+(dyBdz(2:N+1).*dpdz(2:N+1)./P(2:N+1))-(dyBdz(2:N+1).*dTdz(2:N+1)./T(2:N+1)))                        ;
   ...
   % Boundary Derivatives 
    ...
    % dpdt(1)    = lambda*(PressureVaporSat(Tf)-P(1));
    dpdt(end)  = dpdt(end-1);
    dqdt1(1)   = 0;
    dqdt1(end) = 0;
    dqdt2(1)   = 0;
